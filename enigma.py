...
def pass_wheels(input, reverse = False):
    ...
    if(reverse == False):
        Right = SETTINGS["WHEELS"][2]
        input = SETTINGS["WHEEL_POS"][2] + ord(input)

        input = Right["wire"][(input - ord('A'))%26]
        offset = SETTINGS["WHEEL_POS"][1] - SETTINGS["WHEEL_POS"][2]
        input = (ord(input) + offset + 26) % 26

        Middle = SETTINGS["WHEELS"][1]
        input = Middle["wire"][input]
        offset = SETTINGS["WHEEL_POS"][0] - SETTINGS["WHEEL_POS"][1]
        input = (ord(input) + offset + 26) % 26

        Left = SETTINGS["WHEELS"][0]
        input = Left["wire"][input]

    else:
        Left = SETTINGS["WHEELS"][0]
        input = Left["wire"][ord(input) - ord('A')]
        offset = SETTINGS["WHEEL_POS"][0] - SETTINGS["WHEEL_POS"][1]
        input = (ord(input) - offset + 26) % 26
