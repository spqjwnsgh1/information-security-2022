...
def sdes_encrypt_ecb(text: bitarray, key: bitarray):
    block_number = int(len(text)/8)
    result = bitarray()
    for i in range(block_number):
        block = text[i*8:(i+1)*8]
        result += sdes(block,key,MODE_ENCRYPT)

    return result

def sdes_decrypt_ecb(ciphertext: bitarray, key: bitarray):
    block_number = int(len(ciphertext) / 8)
    result = bitarray()
    for i in range(block_number):
        block = ciphertext[i * 8:(i + 1) * 8]
        result += sdes(block, key, MODE_DECRYPT)
...
def sdes_decrypt_cbc(ciphertext: bitarray, key: bitarray, iv:bitarray):
    ...
    for i in range(block_number):
        block = ciphertext[i * 8:(i + 1) * 8]
        block_sdes = sdes(block, key, MODE_DECRYPT)
        result += block_sdes ^ iv
        iv = block
    return result
